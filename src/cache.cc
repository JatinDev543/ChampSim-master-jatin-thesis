...
map <uint64_t,pair<int,int>> ip_accuracy;               //Ip accuracy table
map<uint64_t,int>crit;
//extern int exist(uint64_t);
extern vector <pair<uint64_t,uint64_t>>  ip_lru;
double tprefetch=0;
double thit=0;
double L2C_prefetch=0;
double L2C_hit=0;
double LLC_hit=0;
double LLC_prefetch=0;
double tL2C_prefetch=0;
double tL2C_hit=0;
double tLLC_hit=0;
double tLLC_prefetch=0;

double OLLC_prefetch=0;  //dram prefetches
double OLLC_hit=0;    
//extern double FILL_THRESHOLD;
//extern double PF_THRESHOLD;
...
#ifdef LLC_BYPASS
       // if (((cache_type == IS_LLC) && (way == LLC_WAY))|| ((cache_type == IS_LLC) && (MSHR.entry[mshr_index].confidence>90 )&& (MSHR.entry[mshr_index].type==PREFETCH) )) { // this is a bypass that does not fill the LLC
		...
		if (RQ.entry[index].type == LOAD) {
			...
			cpu = 0;
                ...
                if (block[set][way].prefetch) {
                    pf_useful++;
                    // if(crit[block[set][way].ip]==1)
                    ip_accuracy[block[set][way].ip].first++;                                             //prefetch hit for the ip               
                    block[set][way].prefetch = 0;
                    if(cache_type==IS_LLC)
                     {    LLC_hit++;tLLC_hit++;                                                          //LLC hits
                        //   if(block[set][way].fill_level== FILL_LLC) 
                        //   {
...
void CACHE::fill_cache(uint32_t set, uint32_t way, PACKET *packet)
{
    ...
    block[set][way].fill_level=  packet->fill_level ; 
    ...
    if(block[set][way].prefetch )
      {  
          ip_accuracy[block[set][way].ip].second++;
           if(cache_type==IS_LLC)
              {           LLC_prefetch++;tLLC_prefetch++; 
                        //   if(block[set][way].fill_level==FILL_LLC)
                        //      OLLC_prefetch++;          
              
              }
            else if(cache_type==IS_L2C)
               {          L2C_prefetch++;tL2C_prefetch++;    }
          total_prefetch++;
          tprefetch++;
         
          //crit[block[set][way].ip]=1;
      }   
...
void CACHE::add_mshr(PACKET *packet)
{
    ...
    for (index=0; index<MSHR_SIZE; index++) {
        if (MSHR.entry[index].address == 0) {
            ...
            if(cache_type==IS_LLC &&  MSHR.entry[index].type==PREFETCH )
                    OLLC_prefetch++;
