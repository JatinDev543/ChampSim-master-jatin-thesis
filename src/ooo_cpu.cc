...
double total_entry=0;  //total entries in ROB or total instructions executing.
double crit_delay=0;


double hit=0;
double total_demand=0;
//count currnet core stall cycles
uint64_t current_core_cycle[NUM_CPUS], stall_cycle[NUM_CPUS];
//latency of scheduling and execution
uint32_t SCHEDULING_LATENCY = 0, EXEC_LATENCY = 0;
// lru table to store critical loads
vector <pair<uint64_t,uint64_t>>  ip_lru;
vector <pair<uint64_t,uint64_t>>  ip_lru1;

vector<pair<uint64_t,uint64_t>> ::iterator f(uint64_t x)
{
    ...
    return ip_lru.end();  
                                           ...
                                           //insert IP in lru table
void insert(uint64_t ip,uint64_t diff)
{
    int count=0;
    auto it=f(ip);
    if(it==ip_lru.end())
       {
            if(ip_lru.size()==200)   ip_lru.erase(ip_lru.end()-1);
                    
                
       }
     else  { it->second;ip_lru.erase(it); }
     ip_lru.insert(ip_lru.begin(),make_pair(ip,count));
...
void O3_CPU::handle_branch()
{
    ...
    //read as many instructions as fetch width
        ...
        if (knob_cloudsuite) {
            if (!fread(&current_cloudsuite_instr, instr_size, 1, trace_file)) {
                ...
                for (uint32_t i=0; i<MAX_INSTR_DESTINATIONS; i++) {
                   ...
                   //increment number of register and memory operands
                    if (arch_instr.destination_registers[i])
                        num_reg_ops++;
                    if (arch_instr.destination_memory[i]) {
                        num_mem_ops++;

                        // update STA, this structureadd_to_rob is required to execute store instructios properly without deadlock
                        if (num_mem_ops > 0) {
#ifdef SANITY_CHECK
                ...
                if (ROB.occupancy < ROB.SIZE) {
                    ...
                    num_reads++;//count number of entries in the rob
                    ...
                    if (arch_instr.is_branch) {
                        ...
                        */
                        ...
                        if (arch_instr.branch_taken != branch_prediction) {
                            ...
                            fetch_stall = 1; 
                            //add that branch is mispredicted to rob  
                    ...
                    //if we have fetch enough instructions or we have read enough rob entries then don't read
...
//add instruction to rob man kashturi
uint32_t O3_CPU::add_to_rob(ooo_model_instr *arch_instr)
{
    ...
    ROB.entry[index].event_cycle = current_core_cycle[cpu];
     ROB.entry[index].diff = current_core_cycle[cpu];
     ROB.entry[index].rob_occ=ROB.occupancy;
    // cout<<ROB.occupancy<<endl;
...
//search check rob for the given instruction 
...
//fetch instruction from the rob 
...
void O3_CPU::release_load_queue(uint32_t lq_index)
{
   ...
   // cout <<"Load_queue  event_cycle: " << LQ.entry[lq_index].event_cycle <<"   "<< current_core_cycle[cpu] <<endl; 
...
void O3_CPU::retire_rob()
{
      ...
      for (uint32_t i=0; i<NUM_INSTR_SOURCES; i++) {
        ...
        if (ROB.entry[ROB.head].source_memory[i]) {
            ...
            if(flag==0)
            {
                ...
                ROB.entry[ROB.head].diff=current_core_cycle[cpu]-ROB.entry[ROB.head].diff;
                flag=1;
                p_add=ROB.entry[ROB.head].source_memory[i];
            }
   ...
   if ((ROB.head != ROB.tail) || ROB.occupancy != 0)
    ...
    if (ROB.head < ROB.tail) {
        for (uint32_t i=ROB.head+1; i<ROB.tail; i++) {
            if ( ROB.entry[i].producer_id == ROB.entry[ROB.head].instr_id && ROB.entry[i].executed != COMPLETED) {
                 count++; 
        ...
        for (uint32_t i=ROB.head+1; i<ROB.SIZE; i++) {
              if ( ROB.entry[i].producer_id == ROB.entry[ROB.head].instr_id && ROB.entry[i].executed != COMPLETED) 
              {
                 count++; 
        ...
        for (uint32_t i=0; i<ROB.tail; i++) {
            if ( ROB.entry[i].producer_id == ROB.entry[ROB.head].instr_id && ROB.entry[i].instr_id !=ROB.entry[ROB.head].instr_id && ROB.entry[i].executed != COMPLETED) {
                  count++; 
            }
        }
     ...
     total_entry++;
      ...
      if( ROB.entry[ROB.head].diff >=crit_delay)
      {
          
          insert(ROB.entry[ROB.head].ip,ROB.entry[ROB.head].diff);
         

      }
            //update variables  

      if(count >3 && count <=5) d_5++;
      else if(count >5 && count<=10) {d_10++;    }
      else if(count >10 && count<=15) {d_15++;   }
      else if(count >15 && count <=20) {d_30++;  }
      else if(count >30 && count <=40) {d_40++;  } 
      else if(count >40 && count <=50) {d_50++;  }
      else if(count >50 && count <=100) {d_100++;  } 
      else if(count >100 && count <=150) {d_150++;  }
      else if(count >150 ) { d_192++;         }
      if(count >5)
      {
          
          insert(ROB.entry[ROB.head].ip,count);
          total_entry++;
      }
               
      if(total_entry==100000)                   //refresh delay variables after 100000 load instructions
      {

          total_entry=0;           
      
      long  double sum= r_100+r_200+ r_300+ r_400+r_500+r_600+r_700+r_800+r_900+r_1000;
          if(sum<1000) crit_delay=0;
          else
          {
             r_900+=r_1000;
             r_800+=r_900;
          ...
          int sum1=sum/7;                    //take top (1/7)th critical instructions as threshold for next phase  in this case
          if(r_1000>=sum1) crit_delay=400; 
          else if (r_900>=sum1) crit_delay=300;        
           else if (r_800>=sum1) crit_delay=200;
            else if (r_700>=sum1) crit_delay=150;
             else if (r_600>=sum1) crit_delay=100;
              else if (r_500>=sum1) crit_delay=40;
               else if (r_400>=sum1) crit_delay=20;
                else if (r_300>=sum1) crit_delay=10;
                 else if (r_200>=sum1) crit_delay=5;
                else if (r_100>=sum1) crit_delay=2;
                  else  crit_delay=0;
      ...
      if(ROB.entry[ROB.head].diff >=2 && ROB.entry[ROB.head].diff <=5) r_100++;
      else if(ROB.entry[ROB.head].diff >5 && ROB.entry[ROB.head].diff <=10) r_200++;
      else if(ROB.entry[ROB.head].diff >10 && ROB.entry[ROB.head].diff <=20) r_300++;
      else if(ROB.entry[ROB.head].diff >20 && ROB.entry[ROB.head].diff <=40) r_400++; 
      else if(ROB.entry[ROB.head].diff >40 && ROB.entry[ROB.head].diff <=100) r_500++; 
      else if(ROB.entry[ROB.head].diff >100 && ROB.entry[ROB.head].diff <=150) {r_600++;}  
      else if(ROB.entry[ROB.head].diff >150 && ROB.entry[ROB.head].diff <=200) r_700++; 
      else if(ROB.entry[ROB.head].diff >200 && ROB.entry[ROB.head].diff <=300) r_800++;
      else if(ROB.entry[ROB.head].diff >300 && ROB.entry[ROB.head].diff <=400) r_900++;  
       else if(ROB.entry[ROB.head].diff >400 ) r_1000++;  
        ...
        ooo_model_instr Prev_entry=ROB.entry[ROB.head] ;
        ...
        if (ROB.head == ROB.SIZE)
            ROB.head = 0;
                  ...
                  //initializing rob head cycle to zero if it is load instruction
        for (uint32_t i=0; i<NUM_INSTR_SOURCES; i++) 
        {
        if (ROB.entry[ROB.head].source_memory[i])
              {
           ROB.entry[ROB.head].flag=1;
           ROB.entry[ROB.head].diff=current_core_cycle[cpu];
