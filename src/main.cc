...
extern double hit;
extern double total_demand;
extern double pollution;
extern double total_demandmiss;

extern double L2C_prefetch;
extern double L2C_hit;
extern double LLC_hit;
extern double LLC_prefetch;

extern double OLLC_prefetch;
extern double OLLC_hit;

double cum_ipc=0;
extern  double total_entry;

extern double critical_delay;
extern int critical_depd;
extern int non_critical;
extern int total;

extern double total_hit;
extern double total_prefetch;

double total_miss_L2;
double total_miss_LLC;
...
uint64_t va_to_pa(uint32_t cpu, uint64_t instr_id, uint64_t va, uint64_t unique_vpage)
{
    ...
    //cout <<l1d_prefetcher_operate< " allocated unique_vpage: " << hex << unique_vpage << " to ppage: " << ppage << dec << endl;
    ...
    while (run_simulation) {
        ...
        for (int i=0; i<NUM_CPUS; i++) {
            ...
            */
            ...
            if ((all_warmup_complete > NUM_CPUS) && (simulation_complete[i] == 0) && (ooo_cpu[i].num_retired >= (ooo_cpu[i].begin_sim_instr + ooo_cpu[i].simulation_instructions))) {
                ...
                cout << " (Simulation time: " << elapsed_hour << " hr " << elapsed_minute << " min " << elapsed_second << " sec) " << endl;
               //printing delay and count information
               ...
               // fout << line << endl;  
                // cout<<s1<<endl;
                 int sum=ooo_cpu[i].r_100+ooo_cpu[i].r_200+ooo_cpu[i].r_300+ooo_cpu[i].r_400+ooo_cpu[i].r_500+ooo_cpu[i].r_500+ooo_cpu[i].r_700+ooo_cpu[i].r_800+ooo_cpu[i].r_900+ooo_cpu[i].r_1000;
                
              //   fout1.open("spp_ip/"+s1+".csv",std::ios::app);
 

        string s=ooo_cpu[0].trace_string;
        string s1;
                // finding trace name
                ...
                ofstream fout;
                fout.open("modi_spp/"+s1+".csv");
                fout<<"5 ,"<<(ooo_cpu[i].r_100/sum)*100<<endl;
                fout<<"10 ,"<<(ooo_cpu[i].r_200/sum)*100<<endl;
                fout<<"20 ,"<<(ooo_cpu[i].r_300/sum)*100<<endl;
                fout<<"40 ,"<<(ooo_cpu[i].r_400/sum)*100<<endl;
                fout<<"100 ,"<<(ooo_cpu[i].r_500/sum)*100<<endl;
                fout<<"200 ,"<<(ooo_cpu[i].r_600/sum)*100<<endl;
                fout<<"500 ,"<<(ooo_cpu[i].r_700/sum)*100<<endl;
                fout<<"1000 ,"<<(ooo_cpu[i].r_800/sum)*100<<endl;
                fout<<"1500 ,"<<(ooo_cpu[i].r_900/sum)*100<<endl;
                fout<<"2000 ,"<<(ooo_cpu[i].r_1000/sum)*100<<endl;
                fout.close();

                
           
            
            IP[i]=((float) ooo_cpu[i].finish_sim_instr / ooo_cpu[i].finish_sim_cycle);
        ...
        uncore.DRAM.operate();
                ...
                // finding trace name
                 ...
                 double acc=(total_hit/total_prefetch);
                 
                 
                 fout1.open("spp4/spp1_high_depth3.csv",std::ios::app);
                 fout1<<s1<< "cumulative IPC: ";
                  double t=0;
                  double h=0;
                  double m=0;  
                  double tl=0;
                  ...
                  double ml=0;  
                  for (uint32_t i=0; i<NUM_TYPES; i++) {
                  t += ooo_cpu[0].L2C.sim_access[0][i];
                  h += ooo_cpu[0].L2C.sim_hit[0][i];
                  m += ooo_cpu[0].L2C.sim_miss[0][i];
                          tl +=  uncore.LLC.sim_access[0][i];
                          hl +=  uncore.LLC.sim_hit[0][i];
                          ml +=  uncore.LLC.sim_miss[0][i];
